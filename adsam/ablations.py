"""
Ablation model variants for AD-SAM (used by `python train.py adsam --ablation ...`).
...
from adsam.models.deformable import DeformableConv2d, DeformableFeatureFusion
from adsam.models.attention import ChannelAttention
...
class SAMOnlyDecoder(nn.Module):
    """SAM encoder only (no ResNet branch); plain conv decoder mirroring the AD-SAM stages:
    64->128->256, refine at 256, head at 256x256, logits upsampled x4.
    """
    def __init__(self, sam_model, num_classes=19):
        ...
        self.up3 = self._make_up_block(64, 32)      # refine at 256
    ...
    def forward(self, x, sam_embedding=None):
        ...
        x = self.up1(self._up2(sam_features))       # -> [B, 128, 128, 128]
        x = self.up2(self._up2(x))                  # -> [B, 64, 256, 256]
...
def build_ablation_model(ablation, sam_model, num_classes=19):
    ...
    Args:
        ...
        sam_model: initialized SAM model (or CachedSAMStub)
    ...
    if ablation in ("full", "ce_loss"):
        ...
        model.up3 = StandardUpsampleBlock(64, 32, skip_channels=256, upsample=False)
        return model
