...
class DeformableFeatureFusion(nn.Module):
    ...
    def _init_weights(self):
        for name, m in self.named_modules():
            if not isinstance(m, nn.Conv2d):
                continue
            # DeformableConv2d zero-initialises its offset/mask predictors so the
            # layer starts as a plain 3x3 conv; do not overwrite that here.
            if name.endswith('offset_conv') or name.endswith('mask_conv'):
                continue
    ...
    def forward(self, sam_feat, resnet_feat):
        ...
        # Normalise, then re-weight channels (SE placement: attention after the norm so
        # GroupNorm does not partly undo the per-channel scaling), then activate
